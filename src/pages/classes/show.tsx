...
const Show = () => {
    ...
    const techerName = classDetails.teacher?.name ?? 'Unknown'

    const teacherInitials = techerName.split(' ').filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase()).join('')

    const placeholderUrl = `https://placehold.co/600x400?text=${encodeURIComponent(teacherInitials || 'NA')}`

    const { name, description, status, capacity, bannerUrl, bannerCldPubId, subject, teacher, department, schedules, inviteCode } = classDetails
            ...
            <Card className="details-card">
                ...
                <div className="details-grid">
                    <div className="instructor">
                        ...
                        <div>
                            <img src={teacher?.image ?? placeholderUrl} alt={techerName} />
